...
from fastapi.staticfiles import StaticFiles  # [추가] 정적 파일 처리를 위한 모듈
import os  # [추가] 폴더 확인용
...
# [추가] static 폴더가 없으면 생성 (에러 방지)
if not os.path.exists("static"):
    os.makedirs("static/uploads", exist_ok=True)

# [추가] 정적 파일 경로 마운트 (이 부분이 있어야 이미지가 보입니다!)
# "/static" URL로 요청이 오면, 프로젝트 루트의 "static" 폴더를 찾아보라는 설정
...
app.include_router(ratings_v1.router, prefix="/api/v1/ratings", tags=["ratings"])
...
async def _run_sql_seeds(engine: AsyncEngine) -> None:
    ...
    import re
    # from app.db import db # (상단 import 사용)
